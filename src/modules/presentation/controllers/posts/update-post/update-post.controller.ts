...
import { Can, ScopesOf, makePostGuard } from '../post-guard.js';

export class UpdatePostController {
  constructor(
    private readonly useCase: UpdatePostUseCase,
    private readonly can: Can,
    private readonly scopesOf: ScopesOf,
  ) {}
  ...
  async handle(request: IHttpRequest): Promise<IHttpResponse<UpdatePostOutput>> {
    ...
    const seesWholeSchool = this.scopesOf(actor, Feature.PostUpdate).includes('ESCOLA');
